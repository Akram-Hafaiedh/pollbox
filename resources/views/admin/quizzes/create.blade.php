<x-app-layout>
    ...
    <div class="flex flex-col md:flex-row">
        ...
        <x-dashboard-main-content :page-title="  __('Admin Quizz Creation')">
            <div class="flex items-center justify-between bg-gray-200" {{-- data used in alpine js --}} x-data="{ questions: [{ content: '', options: [''] }], hasCorrectAnswers: true }" x-init="console.log($refs)">

                {{-- x-data="{questions:[{content:'', options:['']}]  ,  hasCorrectAnswers: true }" x-init="console.log($refs)"> --}}
                <form method="post" action="{{ route('admin.quizzes.store') }}" class="w-full p-6">
                    ...
                    <div class="flex mt-4 space-y-3 md:space-y-0 md:flex-row md:items-center">
                        ...
                        <div class="w-full ml-4 md:w-1/4">
                            <div class="mt-2">
                                <input type="checkbox" id="active" name="active" class="w-5 h-5 text-indigo-600 border-gray-300 rounded form-checkbox focus:ring-indigo-500">
                        ...
                        <div class="w-full ml-4 md:w-1/4">
                            ...
                            <div class="mt-2">
                                <input x-model="hasCorrectAnswers" type="checkbox" id="has_correct_answers" name="has_correct_answers" class="w-5 h-5 text-indigo-600 border-gray-300 rounded form-checkbox focus:ring-indigo-500">
                        ...
                        <div class="w-full ml-4 md:w-1/4">
                            <div class="mt-2">
                                <input type="checkbox" id="randomize" name="randomize" class="w-5 h-5 text-indigo-600 border-gray-300 rounded form-checkbox focus:ring-indigo-500">
                                <!-- You can style the checkbox according to your design preferences -->
                            ...
                            <template x-for="(option, optionIndex) in question.options" :key="optionIndex">
                                <div>
                                    ...
                                    <div class="flex flex-row items-center mr-2 space-x-2">
                                        <!-- Input for option-->
                                        <input :id="'option_' + index + '_' + optionIndex" :name="'questions[' + index + '][options][' + optionIndex + '][content]'" class="w-full mt-1 text-sm border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" type="text" required />
                                        <!-- Checkbox for correct answer -->
                                        <template x-if="hasCorrectAnswers" >
                                            <div class="" x-data="{ isCorrect: false }">
                                                <input type="checkbox" :id="'option_' + index + '_' + optionIndex + '_correct'" :name="'questions[' + index + '][options][' + optionIndex + '][is_correct]'" x-model="isCorrect" class="mx-3 text-indigo-600 border-gray-300 rounded w-7 h-7 form-checkbox focus:ring-indigo-500">
                                                <!-- Add a hidden input to store the actual value in the form submission -->
                                                <input type="hidden" :name="'questions[' + index + '][options][' + optionIndex + '][is_correct]'" x-bind:value="isCorrect ? 'true' : 'false'">
                                            </div>
                                        ...
                                        <button type="button" @click="question.options.splice(optionIndex, 1)" class="w-8 h-8 text-white bg-red-500 rounded-md">
